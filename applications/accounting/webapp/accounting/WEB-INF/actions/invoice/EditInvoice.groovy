...
if (invoice) {
    ...
    //Retrieve tax authority product
    Map<String, Object> invoiceTaxAuths = FastMap.newInstance();
    List<GenericValue> invoiceTaxItems = EntityUtil.filterByCondition(invoiceItems,EntityCondition.makeCondition("invoiceItemTypeId", EntityOperator.IN, InvoiceWorker.getTaxableInvoiceItemTypeIds(delegator)));
    for (GenericValue invoiceItem : invoiceTaxItems) {
        //Retrieve related TaxAuthorityRateProduct and calulate amount
        GenericValue taxAuthorityRateProduct = invoiceItem.getRelatedOne("TaxAuthorityRateProduct");
        if (UtilValidate.isNotEmpty(taxAuthorityRateProduct)) {
            GenericValue taxParentInvoiceItem = invoiceItem.getRelatedOne("InvoiceItem");
            String taxAuthorityRate = taxAuthorityRateProduct.getString("taxAuthorityRateSeqId");
            if (!invoiceTaxAuths.containsKey(taxAuthorityRate)) {
                    BigDecimal amountHT = BigDecimal.ZERO;
                    if (taxParentInvoiceItem) {
                        amountHT = InvoiceWorker.getInvoiceItemTotal(taxParentInvoiceItem);
                    }
                    Map<String,Object> taxAuthority = UtilMisc.toMap("description",taxAuthorityRateProduct.getString("description"),"amountHT",amountHT);
                    BigDecimal taxAmount = InvoiceWorker.getTaxTotalForInvoiceItems(UtilMisc.toList(invoiceItem));
                    taxAuthority.put("taxAuthorityRateSeqId",taxAuthorityRate);
                    taxAuthority.put("taxAmount",taxAmount);
                    taxAuthority.put("taxPercentage", taxAuthorityRateProduct.get("taxPercentage"));
                    invoiceTaxAuths.put(taxAuthorityRate, taxAuthority);
            } else {
                Map<String,Object> taxAuthority = invoiceTaxAuths.get(taxAuthorityRate);
                BigDecimal taxAmount = taxAuthority.get("taxAmount");
                taxAuthority.put("taxAmount",taxAmount.add(InvoiceWorker.getTaxTotalForInvoiceItems(UtilMisc.toList(invoiceItem))));
                if (taxParentInvoiceItem) {
                    BigDecimal amountHT = taxAuthority.get("amountHT");
                    taxAuthority.put("amountHT",amountHT.add(InvoiceWorker.getInvoiceItemTotal(taxParentInvoiceItem)));
                }
                invoiceTaxAuths.put(taxAuthorityRate, taxAuthority);
            }
        }
    }
    context.invoiceTaxAuths = invoiceTaxAuths.values();
    //#Eam# invoice-pdf
