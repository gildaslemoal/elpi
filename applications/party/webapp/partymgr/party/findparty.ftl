...
<#if showByPartyType == 'Y'>
  <#if showCustomer == 'Y'>
    <#assign actionFindParty = "FindCustomer"/>
    <h1>${uiLabelMap.PageTitleFindCustomer}</h1>
  </#if>
  <#if showSupplier == 'Y'>
    <#assign actionFindParty = "FindSupplier"/>
    <h1>${uiLabelMap.PageTitleFindSupplier}</h1>
  </#if>
<#else>
  <#assign actionFindParty = "findparty"/>
  ...
  <div class="screenlet-body">
    <div id="findPartyParameters" <#if hideFields != "N"> style="display:none" </#if> >
      ...
      <#-- NOTE: this form is setup to allow a search by partial partyId or userLoginId; to change it to go directly to
       ...
       -->
      <form method="post" name="lookupparty" action="<@ofbizUrl>${actionFindParty}</@ofbizUrl>" class="basic-form">
