...
public class PartyServices {
    ...
    public static Map<String, Object> findParty(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if ("Y".equals(lookupFlag)) {
            ...
            if (!"Y".equals(showAll)) {
                ...
                // filter on MailingType
                String mailingType = (String) context.get("mailingType");
                if (UtilValidate.isNotEmpty(mailingType)) {
                    if ("newsletter".equals(mailingType)) {
                        paramList = paramList + "&mailingType=" + mailingType;

                        // modify the dynamic view
                        dynamicView.addMemberEntity("PCM", "PartyContactMech");
                        dynamicView.addAlias("PCM", "allowSolicitation");
                        dynamicView.addViewLink("PT", "PCM", Boolean.FALSE, ModelKeyMap.makeKeyMapList("partyId"));

                        // add the expr
                        andExprs.add(EntityCondition.makeCondition("allowSolicitation", EntityOperator.EQUALS, "Y"));

                        fieldsToSelect.add("allowSolicitation");
                    } else if ("cotisation".equals(mailingType)) {
                        paramList = paramList + "&mailingType=" + mailingType;

                        // modify the dynamic view
                        dynamicView.addMemberEntity("PAT", "PartyAttribute");
                        dynamicView.addAlias("PAT", "attrName");
                        dynamicView.addAlias("PAT", "attrValue");
                        dynamicView.addViewLink("PT", "PAT", Boolean.FALSE, ModelKeyMap.makeKeyMapList("partyId"));

                        // add the expr
                        andExprs.add(EntityCondition.makeCondition("attrName", EntityOperator.EQUALS, "COTISATION"));
                        andExprs.add(EntityCondition.makeCondition("attrValue", EntityOperator.NOT_EQUAL, "1"));

                        fieldsToSelect.add("attrName");
                        fieldsToSelect.add("attrValue");
                    } else if ("codepostal".equals(mailingType)) {
    ...
    /**
     * Create customer company as Party Group, with postal address, email and phone number
     * @param ctx the dispatch context
     * @param context Map containing the input parameters.
     * @return Map with the result of the service, the output parameters.
     */
    public static Map<String, Object> createPartyGroupCustomer(DispatchContext ctx, Map<String, Object> context) {
        LocalDispatcher dispatcher = ctx.getDispatcher();
        Map<String, Object> result = ServiceUtil.returnSuccess();
        ...
        try {
            // Create Party group
            ModelService createPartyGroupService = ctx.getModelService("createPartyGroup");
            serviceContext = createPartyGroupService.makeValid(context, ModelService.IN_PARAM);
            ...
            // Add Customer and Internal Organisation roles
            serviceContext.clear();
            serviceContext.put("userLogin", userLogin);
            serviceContext.put("partyId", partyId);
            serviceContext.put("roleTypeId", "CUSTOMER");
            result = dispatcher.runSync("createPartyRole", serviceContext);
            if (ServiceUtil.isError(result)) {
                return result;
            }
            serviceContext.put("roleTypeId", "INTERNAL_ORGANIZATIO");
            result = dispatcher.runSync("createPartyRole", serviceContext);
            if (ServiceUtil.isError(result)) {
                return result;
            }
            ...
            if (! UtilValidate.isEmpty(address1) && ! UtilValidate.isEmpty(city) && ! UtilValidate.isEmpty(postalCode)) {
                serviceContext.clear();
                ModelService createPartyPostalAddressService = ctx.getModelService("createPartyPostalAddress");
                serviceContext = createPartyPostalAddressService.makeValid(context, ModelService.IN_PARAM);
                serviceContext.put("partyId", partyId);
                result = dispatcher.runSync("createPartyPostalAddress", serviceContext);
                if (ServiceUtil.isError(result)) {
                    return result;
                }
                // Add customer address purposes
                String contactMechId = (String) result.get("contactMechId");
                serviceContext.clear();
                serviceContext.put("userLogin", userLogin);
                serviceContext.put("partyId", partyId);
                serviceContext.put("contactMechId", contactMechId);
                serviceContext.put("contactMechPurposeTypeId", "GENERAL_LOCATION");
                result = dispatcher.runSync("createPartyContactMechPurpose", serviceContext);
                if (ServiceUtil.isError(result)) {
                    return result;
                }
            ...
            if (! UtilValidate.isEmpty(contactNumber)) {
                serviceContext.clear();
                ModelService createPartyTelecomNumberService = ctx.getModelService("createPartyTelecomNumber");
                serviceContext = createPartyTelecomNumberService.makeValid(context, ModelService.IN_PARAM);
                serviceContext.put("partyId", partyId);
            ...
            if (! UtilValidate.isEmpty(emailAddress)) {
                serviceContext.clear();
                ModelService createPartyEmailAddressService = ctx.getModelService("createPartyEmailAddress");
                serviceContext = createPartyEmailAddressService.makeValid(context, ModelService.IN_PARAM);
                serviceContext.put("partyId", partyId);
            ...
            String msg = "Can not create customer party group due to following error: " + e.getLocalizedMessage();
    ...
    /**
     * Create customer contact linked to a customer company
     * @param ctx the dispatch context
     * @param context Map containing the input parameters.
     * @return Map with the result of the service, the output parameters.
     */
    public static Map<String, Object> createCustomerContact(DispatchContext ctx, Map<String, Object> context) {
        LocalDispatcher dispatcher = ctx.getDispatcher();
        Map<String, Object> result = ServiceUtil.returnSuccess();
        Map<String, Object> serviceContext = FastMap.newInstance();
        String partyId = null;
        ...
        try {
            // Create Party group
            ModelService createPersonService = ctx.getModelService("createPerson");
            serviceContext = createPersonService.makeValid(context, ModelService.IN_PARAM);
            result = dispatcher.runSync("createPerson", serviceContext);
            if (ServiceUtil.isError(result)) {
                return result;
            }
            ...
            // Add Customer and Employee roles
            serviceContext.clear();
            serviceContext.put("userLogin", userLogin);
            serviceContext.put("partyId", partyId);
            serviceContext.put("roleTypeId", "CUSTOMER");
            ...
            serviceContext.put("roleTypeId", "EMPLOYEE");
            result = dispatcher.runSync("createPartyRole", serviceContext);
            if (ServiceUtil.isError(result)) {
                return result;
            }
            
            // Create link between the employee and the company
            serviceContext.clear();
            serviceContext.put("userLogin", userLogin);
            serviceContext.put("partyIdFrom", customerPartyId);
            serviceContext.put("partyIdTo", partyId);
            serviceContext.put("roleTypeIdFrom", "INTERNAL_ORGANIZATIO");
            serviceContext.put("roleTypeIdTo", "EMPLOYEE");
            serviceContext.put("partyRelationshipTypeId", "EMPLOYMENT");
            ...
            String msg = "Can not create customer contact due to following error: " + e.getLocalizedMessage();
            Debug.logError(msg, module);
            return ServiceUtil.returnError(msg);
        }
    ...
    /**
     * Create supplier company as Party Group, with postal address, email and phone number
     * @param ctx the dispatch context
     * @param context Map containing the input parameters.
     * @return Map with the result of the service, the output parameters.
     */
    public static Map<String, Object> createPartyGroupSupplier(DispatchContext ctx, Map<String, Object> context) {
        LocalDispatcher dispatcher = ctx.getDispatcher();
        Map<String, Object> result = ServiceUtil.returnSuccess();
        Map<String, Object> serviceContext = FastMap.newInstance();
        String partyId = null;
        ...
        try {
            ...
            // Add Supplier and Internal Organisation roles
            serviceContext.clear();
            serviceContext.put("userLogin", userLogin);
            serviceContext.put("partyId", partyId);
            ...
            String msg = "Can not create supplier party group due to following error: " + e.getLocalizedMessage();
            Debug.logError(msg, module);
            return ServiceUtil.returnError(msg);
        }
     ...
     * @return Map with the result of the service, the output parameters.
     */
    public static Map<String, Object> createSupplierContact(DispatchContext ctx, Map<String, Object> context) {
        LocalDispatcher dispatcher = ctx.getDispatcher();
        Map<String, Object> result = ServiceUtil.returnSuccess();
        Map<String, Object> serviceContext = FastMap.newInstance();
        String partyId = null;
        ...
        try {
            ...
            serviceContext.clear();
            serviceContext.put("userLogin", userLogin);
            serviceContext.put("partyIdFrom", supplierPartyId);
            serviceContext.put("partyIdTo", partyId);
            serviceContext.put("roleTypeIdFrom", "INTERNAL_ORGANIZATIO");
            serviceContext.put("roleTypeIdTo", "EMPLOYEE");
            serviceContext.put("partyRelationshipTypeId", "EMPLOYMENT");
