...
public class PriceServices {
    ...
    public static Map<String, Object> calculateProductPrice(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        String productOfferId = product.getString("productOfferId");
        ...
        GenericValue defaultPriceValue = null;
        if (UtilValidate.isNotEmpty(productOfferId)) {
            GenericValue productOffer = null;
            try {
                productOffer = delegator.findOne("ProductOffer", UtilMisc.toMap("productOfferId", productOfferId), false);
            } catch (GenericEntityException e) {
                Debug.logError(e, "An error occurred while getting the product offer price", module);
            }
            defaultPriceValue = delegator.makeValidValue("ProductPrice", UtilMisc.toMap("price", productOffer.get("consumerSalePrice")));
        } else {
            defaultPriceValue = getPriceValueForType("DEFAULT_PRICE", productPrices, virtualProductPrices);
        }
