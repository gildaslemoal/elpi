...
<div id="orderLookup" class="screenlet">
    ...
    <div class="screenlet-body">
      <#assign showByOrderType = showByOrderType?if_exists/>
      <#assign showSalesOrder = showSalesOrder?if_exists/>
      <#if showByOrderType == 'Y'>
        <#if showSalesOrder == 'Y'>
          <#assign actionOrderList = "SalesOrderlist"/>
        <#else>
          <#assign actionOrderList = "PurchaseOrderlist"/>
        </#if>
      <#else>
        <#assign actionOrderList = "orderlist"/>
      </#if>
      <form method="post" name="findorder" action="<@ofbizUrl>${actionOrderList}</@ofbizUrl>">
        ...
        <table class="basic-table" cellspacing='0'>
          ...
          <tr>
            <#if showByOrderType == 'Y'>
              <#if showSalesOrder == 'Y'>
                <input type="checkbox" name="view_SALES_ORDER" value="Y" checked="checked" hidden="true"/>
                <input type="checkbox" name="view_PURCHASE_ORDER" value="N" hidden="true"/>
              <#else>
                <input type="checkbox" name="view_SALES_ORDER" value="N" hidden="true"/>
                <input type="checkbox" name="view_PURCHASE_ORDER" value="Y" checked="checked" hidden="true"/>
              </#if>
            <#else>
