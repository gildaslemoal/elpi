...
<#macro renderTextField name className alert value textSize maxlength id event action disabled clientAutocomplete ajaxUrl ajaxEnabled mask>
  ...
  <input class="form-control" type="text" name="${name?default("")?html}"<#t/>
...
<#macro renderDropDownField name className alert id multiple formName otherFieldName event action size firstInList currentValue explicitDescription allowEmpty options fieldName otherFieldName otherValue otherFieldSize dDFCurrent ajaxEnabled noCurrentSelectedKey ajaxOptions frequency minChars choices autoSelect partialSearch partialChars ignoreCase fullSearch>
  <span class="ui-widget">
    <select class="selectpicker" name="${name?default("")}<#rt/>" <@renderClass className alert /><#if id?has_content> id="${id}"</#if><#if multiple?has_content> multiple="multiple"</#if><#if otherFieldSize gt 0> onchange="process_choice(this,document.${formName}.${otherFieldName})"</#if><#if event?has_content> ${event}="${action}"</#if><#if size?has_content> size="${size}"</#if>>
...
<#macro renderSubmitField buttonType className alert formName title name event action imgSrc confirmation containerId ajaxUrl>
  ...
  <#else>
    <input class="btn btn-primary" type="<#if containerId?has_content>button<#else>submit</#if>" <@renderClass className alert />
...
<#macro renderFieldTitle style title id fieldHelpText="">
  <span class="input-group-addon<#if style?has_content> ${style}</#if>" <#if fieldHelpText?has_content> title="${fieldHelpText}"</#if><#if id?has_content> id="${id}"</#if>><#t/>
    ${title}<#t/>
  </span><#t/>
</#macro>
...
<#macro renderFormOpen linkUrl formType targetWindow containerId containerStyle autocomplete name viewIndexField viewSizeField viewIndex viewSize useRowSubmit>
  <form method="post" action="${linkUrl}"<#if formType=="upload"> enctype="multipart/form-data"</#if><#if targetWindow?has_content> target="${targetWindow}"</#if><#if containerId?has_content> id="${containerId}"</#if> class=<#if containerStyle?has_content>"${containerStyle}"<#else>"form-horizontal"</#if> onsubmit="javascript:submitFormDisableSubmits(this)"<#if autocomplete?has_content> autocomplete="${autocomplete}"</#if> name="${name}"><#lt/>
...
<#macro renderFormatListWrapperOpen formName style columnStyles>
  <table cellspacing="20" class="<#if style?has_content>${style} table table-striped table-hover table-bordered<#else>basic-table form-widget-table dark-grid</#if>"> <#if title?has_content><caption class="text-right"><h3>${title}</h3></caption></#if><#lt/>
</#macro>
...
<!--   <table cellspacing="30" <#if style?has_content>class="${style}"</#if>> -->
</#macro>
...
<#macro renderLookupField name formName fieldFormName className="" alert="false" value="" size="" maxlength="" id="" event="" action="" readonly=false autocomplete="" descriptionFieldName="" targetParameterIter="" imgSrc="" ajaxUrl="" ajaxEnabled=javaScriptEnabled presentation="layer" width="" height="" position="" fadeBackground="true" clearText="" showDescription="" initiallyCollapsed="" lastViewName="main" >
  ...
  <#if ajaxEnabled?has_content && ajaxEnabled>
    ...
    <#else>
      <input type="text" class="form-control" <@renderClass className alert /><#if name?has_content> name="${name}"</#if><#if value?has_content> value="${value}"</#if>
    ...
    <#if presentation?has_content && descriptionFieldName?has_content && presentation == "window">
      <button class="btn btn-default glyphicon glyphicon-list" type="button">
