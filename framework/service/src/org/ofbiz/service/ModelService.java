...
import java.util.HashMap; //#Eam# new-wsdl-style
...
public class ModelService extends AbstractMap<String, Object> implements Serializable {
    ...
    */
    ...
    public void buildWSDL(Delegator delegator, Definition def, String locationURI) throws WSDLException {
        ...
        */
        ...
        Types types = this.buildTypes(document, def, refs);
        def.setTypes(types);
        ...
        if (inParam != null) {
            ...
            inMessage.setUndefined(false);
            Part parametersPart = def.createPart();
            parametersPart.setName("parameters");
            parametersPart.setElementName(new QName(TNS, this.name));
            inMessage.addPart(parametersPart);
            Element requestParameters = (Element) refs.get("in");
            //types.getDocumentationElement().appendChild(requestParameters);
            for (String paramName: inParam) {
                ModelParam param = this.getParam(paramName);
                if (!param.internal) {
                    Element para = document.createElement("xsd:element");
                    if (param.optional) {
                        para.setAttribute("minOccurs", "0");
                    }
                    else {
                        para.setAttribute("minOccurs", "1");
                    }
                    para.setAttribute("maxOccurs", "1");
                    para.setAttribute("name", param.name);
                    if (ObjectType.instanceOf(java.util.List.class, param.type)) {
                        addListType(document, delegator, types.getDocumentationElement(), param);
                        para.setAttribute("type", "tns:" + param.name + "List");
                    } else if (ObjectType.instanceOf(java.util.Map.class, param.type)){
                        addMapType(document, delegator, types.getDocumentationElement(), param);
                        para.setAttribute("type", "tns:" + param.name + "Map");
                    }
                    else {
                        para.setAttribute("type", param.toWsdlType());
                    }
                    requestParameters.appendChild(para);
                }
            }
            Element usernameAttr = document.createElement("xsd:element");
            usernameAttr.setAttribute("name", "login.username");
            usernameAttr.setAttribute("type", "xsd:string");
            usernameAttr.setAttribute("maxOccurs", "1");
            usernameAttr.setAttribute("minOccurs", "1");
            requestParameters.appendChild(usernameAttr);

            Element passwordAttr = document.createElement("xsd:element");
            passwordAttr.setAttribute("name", "login.password");
            passwordAttr.setAttribute("type", "xsd:string");
            passwordAttr.setAttribute("maxOccurs", "1");
            passwordAttr.setAttribute("minOccurs", "1");
            requestParameters.appendChild(passwordAttr);
        ...
        if (outParam != null) {
            ...
            outMessage.setUndefined(false);
            Part resultsPart = def.createPart();
            resultsPart.setName("parameters");
            resultsPart.setElementName(new QName(TNS, this.name + "Response"));
            Element responseParameters = (Element) refs.get("out");
            outMessage.addPart(resultsPart);
            //types.getDocumentationElement().appendChild(responseParameters);
            for (String paramName: outParam) {
                ModelParam param = this.getParam(paramName);
                if (!param.internal) {
                    Element para = document.createElement("xsd:element");
                    ...
                    responseParameters.appendChild(para);
                }
            }
            def.addMessage(outMessage);
            output.setMessage(outMessage);
    ...
    public void addSystemOutParams(Element response) {
        prepareSystemOutParams(response, RESPONSE_MESSAGE);
        prepareSystemOutParams(response, ERROR_MESSAGE);
        prepareSystemOutParams(response, ERROR_MESSAGE_LIST);
        prepareSystemOutParams(response, ERROR_MESSAGE_MAP);
        prepareSystemOutParams(response, SUCCESS_MESSAGE);
        prepareSystemOutParams(response, SUCCESS_MESSAGE_LIST);
    }

    public void prepareSystemOutParams(Element response, String name) {
        Element param = response.getOwnerDocument().createElement("xsd:element");
        param.setAttribute("name", name);
        param.setAttribute("minOccurs", "0");
        param.setAttribute("maxOccurs", "1");
        param.setAttribute("type", "xsd:string");
        response.appendChild(param);
    }

    public void addListType(Document document, Delegator delegator, Element parent, ModelParam param) throws WSDLException{
        ModelFieldTypeReader mftr = ModelFieldTypeReader.getModelFieldTypeReader(delegator.getGroupHelperInfo("org.ofbiz").getHelperBaseName());
        Element listType = document.createElement("xsd:complexType");
        listType.setAttribute("name", param.name + "List");
        Element sequence = document.createElement("xsd:sequence");
        listType.appendChild(sequence);
        Element listElement = document.createElement("xsd:element");
        listElement.setAttribute("minOccurs", "0");
        listElement.setAttribute("maxOccurs", "unbounded");
        listElement.setAttribute("name", param.name + "-entry");
        listElement.setAttribute("type", "tns:" + param.name + "-entry");
        sequence.appendChild(listElement);
        
        parent.appendChild(listType);
        Element listEntryType = document.createElement("xsd:complexType");
        listEntryType.setAttribute("name", param.name + "-entry");
        Element listEntryTypeSequence = document.createElement("xsd:all");
        listEntryType.appendChild(listEntryTypeSequence);
        parent.appendChild(listEntryType);
        
        if (param.getAttrList() != null) {
            ModelParamAttrList attrList = param.getAttrList();
            List<ModelParamAttrMapEntry> entries = attrList.getEntries();
            if (UtilValidate.isNotEmpty(entries)) {
                for (ModelParamAttrMapEntry entry : entries) {
                    String type = entry.type;
                    Element oneElement = document.createElement("xsd:element");
                    if (entry.optional) {
                        oneElement.setAttribute("minOccurs", "0");
                    }
                    else {
                        oneElement.setAttribute("minOccurs", "1");
                    }
                    oneElement.setAttribute("maxOccurs", "1");
                    oneElement.setAttribute("nillable", "true");
                    oneElement.setAttribute("name", entry.name);
                    
                    //find java type
                    String javaType = null;
                    try {
                        javaType = toWsdlType(type);
                    }
                    catch(Exception e) {
                        //just ignore it
                    }
                    if(javaType == null){
                        javaType = toWsdlType(mftr.getModelFieldType(type).getJavaType());
                    }
                    oneElement.setAttribute("type", javaType);
                    listEntryTypeSequence.appendChild(oneElement);
                }
            }
        }
    }

    public void addMapType(Document document, Delegator delegator, Element parent, ModelParam param) throws WSDLException{
        ModelFieldTypeReader mftr = ModelFieldTypeReader.getModelFieldTypeReader(delegator.getGroupHelperInfo("org.ofbiz").getHelperBaseName());
        Element mapType = document.createElement("xsd:complexType");
        mapType.setAttribute("name", param.name + "Map");
        Element mapTypeSequence = document.createElement("xsd:all");
        mapType.appendChild(mapTypeSequence);
        parent.appendChild(mapType);
        if (param.getAttrMap() != null) {
            List<ModelParamAttrMapEntry> entries = param.getAttrMap().getEntries();
            if (UtilValidate.isNotEmpty(entries)) {
                for (ModelParamAttrMapEntry entry : entries) {
                    String type = entry.type;
                    Element oneElement = document.createElement("xsd:element");
                    ...
                    oneElement.setAttribute("type", javaType);
                    mapTypeSequence.appendChild(oneElement);
                }
            }
        }
    }

    public String toWsdlType(String thisType) throws WSDLException {
        if (ObjectType.instanceOf(java.lang.Character.class, thisType)) {
            return "xsd:string";
        } else if (ObjectType.instanceOf(java.lang.String.class, thisType)) {
            return "xsd:string";
        } else if (ObjectType.instanceOf(java.lang.Byte.class, thisType)) {
            return "xsd:string";
        } else if (ObjectType.instanceOf(java.lang.Boolean.class, thisType)) {
            return "xsd:boolean";
        } else if (ObjectType.instanceOf(java.lang.Integer.class, thisType)) {
            return "xsd:integer";
        } else if (ObjectType.instanceOf(java.lang.Double.class, thisType)) {
            return "xsd:double";
        } else if (ObjectType.instanceOf(java.lang.Float.class, thisType)) {
            return "xsd:float";
        } else if (ObjectType.instanceOf(java.lang.Short.class, thisType)) {
            return "xsd:integer";
        } else if (ObjectType.instanceOf(java.math.BigDecimal.class, thisType)) {
            return "xsd:double";
        } else if (ObjectType.instanceOf(java.math.BigInteger.class, thisType)) {
            return "xsd:integer";
        } else if (ObjectType.instanceOf(java.util.Calendar.class, thisType)) {
            return "xsd:dateTime";
        } else if (ObjectType.instanceOf(com.ibm.icu.util.Calendar.class, thisType)) {
            return "xsd:dateTime";
        } else if (ObjectType.instanceOf(java.sql.Date.class, thisType)) {
            return "xsd:date";
        } else if (ObjectType.instanceOf(java.util.Date.class, thisType)) {
            return "xsd:dateTime";
        } else if (ObjectType.instanceOf(java.lang.Long.class, thisType)) {
            return "xsd:long";
        } else if (ObjectType.instanceOf(java.sql.Timestamp.class, thisType)) {
            return "xsd:dateTime";
        } else {
            return "xsd:string";
        }
