...
public class ModelParam implements Serializable {
    ...
    /*#Bam# new-wsdl-style*/
    public ModelParamAttrMap attrMap = null;
    public ModelParamAttrList attrList = null;
    /*#Eam# new-wsdl-style*/
    public ModelParam() {}
    ...
    /*#Bam# new-wsdl-style*/
    protected String toWsdlType() throws WSDLException {
        if (ObjectType.instanceOf(java.lang.Character.class, this.type)) {
            return "xsd:string";
        } else if (ObjectType.instanceOf(java.lang.String.class, this.type)) {
            return "xsd:string";
    ...
    public static class ModelParamAttrMapEntry {
        String name;
        String type;
        boolean optional = true;
        public ModelParamAttrMapEntry(String name, String type, boolean optional) {
            this.name = name;
            this.type = type;
    ...
    public static class EntityMap {
        ...
        boolean optional = true;
        public EntityMap(String entityName, String include, List<String> excludList, boolean optional) {
            this.entityName = entityName;
            this.include = include;
            this.excludList = excludList;
            this.optional = optional;
        }
    }

    public static class ModelParamAttrMap {
        List<ModelParamAttrMapEntry> entries = null;
        public ModelParamAttrMap(Element mapElement, Delegator delegator){
            this.initEntries();
            for(Element mapEntry : UtilXml.childElementList(mapElement, "map-entry")) {
                ModelParamAttrMapEntry entry = new ModelParamAttrMapEntry(mapEntry.getAttribute("name"), mapEntry.getAttribute("type"), "true".equals(mapEntry.getAttribute("type")));
                this.addEntry(entry);
            }
            for(Element entityMap : UtilXml.childElementList(mapElement, "entity-map")) {
                List<String> excludList = new ArrayList<String>();
                String entityName = entityMap.getAttribute("entity-name");
                String include = "all";
                if (UtilValidate.isNotEmpty(entityMap.getAttribute("include"))) {
                    include = entityMap.getAttribute("include");
                }
                boolean optional = false;
                if (UtilValidate.isNotEmpty(entityMap.getAttribute("optional"))) {
                    optional = "true".equals(entityMap.getAttribute("optional"));
                }
                for(Element toExclude : UtilXml.childElementList(entityMap, "exclude")) {
                    excludList.add(toExclude.getAttribute("field-name"));
                }

                ModelEntity modelEntity = delegator.getModelEntity(entityName);
                List<ModelField> fields = new ArrayList<ModelField>();
                if ("pk".equals(include)) {
                    for (ModelField modelField :modelEntity.getPkFieldsUnmodifiable()) {
                        if (UtilValidate.isNotEmpty(excludList) && excludList.contains(modelField.getName())) {
                            fields.add(modelField);
                        }
                    }
                }
                else if ("nonpk".equals(include)) {
                    Iterator<ModelField> modelFields = modelEntity.getNopksIterator();
                ...
                for (ModelField field : fields) {
                    ModelParamAttrMapEntry entry = new ModelParamAttrMapEntry(field.getName(), field.getType(), optional);
                    entries.add(entry);
                }
            }
        }

        public ModelParamAttrMapEntry getEntry(String name) {
        ...
        public List<ModelParamAttrMapEntry> getEntries() {
            return entries;
        }

        public void addEntry(ModelParamAttrMapEntry entry){
            entries.add(entry);
        }

        public boolean emptyEntries() {
            return UtilValidate.isEmpty(entries);
        }
        ...
        public void initEntries() {
            entries = new ArrayList<ModelParamAttrMapEntry>();
        }
    }

    public static class ModelParamAttrList {
        ModelParamAttrMap map;

        public ModelParamAttrList(Element listElement, Delegator delegator){
            ModelParamAttrMap map = null;
            for(Element mapElement : UtilXml.childElementList(listElement, "map")) {
                map = new ModelParamAttrMap(mapElement, delegator);
            }
            this.map = map;
        }
        ...
        public ModelParamAttrMap getModelParamAttrMap() {
            return this.map;
        }
    }

    public void setAttrMap(Element mapElement, Delegator delegator) {
        this.attrMap = new ModelParamAttrMap(mapElement, delegator);
    }

    public ModelParamAttrMap getAttrMap() {
        return this.attrMap;
    }

    public void setAttrList(Element listElement, Delegator delegator) {
        this.attrList = new ModelParamAttrList(listElement, delegator);
    }

    public ModelParamAttrList getAttrList() {
        return this.attrList;
    }

    public boolean hasListStructure() {
        return UtilValidate.isNotEmpty(getAttrList());
    }

    public boolean hasMapStructure() {
       return UtilValidate.isNotEmpty(getAttrMap());
    }

    public Object getModelParamAttrDef() {
